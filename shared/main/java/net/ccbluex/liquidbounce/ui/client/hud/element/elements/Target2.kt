        ...
        if (riceParticle.get()) {
            ...
            if (gotDamaged) {
                for (j in 0..(generateAmountValue.get())) {
                    ...
                    val hud = LiquidBounce.moduleManager[Gident::class.java] as Gident
                    val color2 = RenderUtils.getGradientOffset(
                        Color(Gident.redValue.get(), Gident.greenValue.get(), Gident.blueValue.get()),
                        Color(Gident.redValue2.get(), Gident.greenValue2.get(), Gident.blueValue2.get()),
                        10.0
                    )
                    particleList.add(
                        Particle(
...
//        GL11.glTranslated(renderX, renderY, 0.0)
    ...
    private fun drawMine(target: IEntityLivingBase,easingHealth: Float){
        val font = FontLoaders.F24
