...
class GuiMainMenu : WrappedGuiScreen() {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        val sr = ScaledResolution(MinecraftInstance.mc2)
        val res = ScaledResolution(MinecraftInstance.mc2);
        ...
        FontLoaders.F16.drawStringWithShadow(" ${LiquidBounce.CLIENT_NAME} build ${LiquidBounce.CLIENT_VERSION} | By 神风",
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
        ...
        if (isMouseHover(2F, representedScreen.height - 26F, 28F, representedScreen.height - 16F, mouseX, mouseY))
            useParallax = !useParallax

        if (isMouseHover(2F, representedScreen.height - 38F, 28F, representedScreen.height - 28F, mouseX, mouseY))
            LiquidBounce.darkMode = !LiquidBounce.darkMode

        val staticX = representedScreen.width / 2F - 120F
        val staticY = representedScreen.height / 2F + 20F
        var index: Int = 0
        for (icon in if (extendedModMode) ExtendedImageButton.values() else ImageButton.values()) {
            if (isMouseHover(staticX + 40F * index, staticY, staticX + 40F * (index + 1), staticY + 20F, mouseX, mouseY))
                when (index) {
                    ...
                    1 -> if (extendedBackgroundMode) GuiBackground.enabled = !GuiBackground.enabled else if (extendedModMode) mc.displayGuiScreen(classProvider.wrapGuiScreen(GuiModsMenu(this.representedScreen))) else mc.displayGuiScreen(classProvider.createGuiMultiplayer(this.representedScreen))
                    2 -> if (extendedBackgroundMode) GuiBackground.particles = !GuiBackground.particles else if (extendedModMode) mc.displayGuiScreen(classProvider.wrapGuiScreen(GuiModsMenu(this.representedScreen))) else mc.displayGuiScreen(classProvider.wrapGuiScreen(GuiAltManager(this.representedScreen)))
    ...
    fun moveMouseEffect(mouseX: Int, mouseY: Int, strength: Float) {
        val mX = mouseX - representedScreen.width / 2
        val mY = mouseY - representedScreen.height / 2
        val xDelta = mX.toFloat() / (representedScreen.width / 2).toFloat()
        val yDelta = mY.toFloat() / (representedScreen.height / 2).toFloat()
    ...
    fun renderSwitchButton() {
        ...
        FontLoaders.F16.drawString("Parallax", 28.0F, (representedScreen.height - 25F).toDouble().toFloat(), -1,true)
        RenderUtils.drawRoundedRect(4F, representedScreen.height - 24F, 22F, representedScreen.height - 18F, 3, if (useParallax) Color(0, 111, 255, 255).rgb else (if (LiquidBounce.darkMode) Color(70, 70, 70, 255) else Color(140, 140, 140, 255)).rgb)
        RenderUtils.drawRoundedRect(2F + sliderX, representedScreen.height - 26F, 12F + sliderX, representedScreen.height - 16F, 5, Color.white.rgb)
    }

    fun renderDarkModeButton() {
        sliderDarkX = (sliderDarkX + (if (LiquidBounce.darkMode) 2F else -2F)).coerceIn(0F, 12F)
        GlStateManager.disableAlpha()
        RenderUtils.drawImage3(darkIcon, 28F, representedScreen.height - 40F, 14, 14, 1F, 1F, 1F, sliderDarkX / 12F)
        RenderUtils.drawImage3(lightIcon, 28F, representedScreen.height - 40F, 14, 14, 1F, 1F, 1F, 1F - (sliderDarkX / 12F))
        GlStateManager.enableAlpha()
        RenderUtils.drawRoundedRect(4F, representedScreen.height - 36F, 22F, representedScreen.height - 30F, 3, (if (LiquidBounce.darkMode) Color(70, 70, 70, 255) else Color(140, 140, 140, 255)).rgb)
        RenderUtils.drawRoundedRect(2F + sliderDarkX, representedScreen.height - 38F, 12F + sliderDarkX, representedScreen.height - 28F, 5, Color.white.rgb)
    }
    ...
    fun renderBar(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        RenderUtils.drawRoundedRect(staticX, staticY, staticX + 240F, staticY + 20F, 10, (if (LiquidBounce.darkMode) Color(0, 0, 0, 100) else Color(255, 255, 255, 100)).rgb)
        ...
        if (displayString != null)
            FontLoaders.F16.drawCenteredString(displayString!!,
                (representedScreen.width / 2F).toDouble(), (staticY + 30F).toDouble(), -1)
        else
            FontLoaders.F16.drawCenteredString("你好哟!", (representedScreen.width / 2F).toDouble(),
        ...
        if (fade != 0F)
            RenderUtils.drawRoundedRect(slideX, staticY, slideX + 40F, staticY + 20F, 10, (if (LiquidBounce.darkMode) Color(0F, 0F, 0F, fade / 100F * 0.6F) else Color(1F, 1F, 1F, fade / 100F * 0.6F)).rgb)
            ...
            if (extendedBackgroundMode)
                for (i in ExtendedBackgroundButton.values()) {
                    if (LiquidBounce.darkMode)
                        RenderUtils.drawImage2(i.texture, (staticX + 40F * index + 11F).toInt(), (staticY + 1F).toInt(), 18, 18)
                    else
