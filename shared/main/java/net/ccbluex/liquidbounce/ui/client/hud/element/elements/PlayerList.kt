...
class PlayerList : Element() {
    ...
    override fun drawElement(): Border {
        ...
        val font = FontLoaders.F16
        val fontOffset = fontOffsetValue.get()
        val rainbowType = rainbowList.get()

        var nameLength = font.getStringWidth("Name (0)").toFloat()
        var hpLength = font.getStringWidth("健康").toFloat()
        var distLength = font.getStringWidth("距离").toFloat()

        var height = 4F + FontLoaders.F16.height
        ...
        nameLength = font.getStringWidth("名字 (${playerList.size})").toFloat()
        ...
        RenderUtils.drawRect(0F, 0F, nameLength + hpLength + distLength + 50F, 4F + FontLoaders.F16.height, bgColor.rgb)

        font.drawString("名字 (${playerList.size})", 5F, 3F, -1, shadowValue.get())
        font.drawString("距离", 5F + nameLength + 10F, 3F, -1, shadowValue.get())
        font.drawString("健康", 5F + nameLength + distLength + 20F, 3F, -1, shadowValue.get())

        playerList.forEachIndexed { index, player ->
            RenderUtils.drawRect(0F, height, nameLength + hpLength + distLength + 50F, height + 2F + FontLoaders.F16.height, bgColor.rgb)
            ...
            height += 2F + FontLoaders.F16.height
        }

        return Border(0F, 0F, nameLength + hpLength + distLength + 50F, 4F + height + FontLoaders.F16.height)
    }
}