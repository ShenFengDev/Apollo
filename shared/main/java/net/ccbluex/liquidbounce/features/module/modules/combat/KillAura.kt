...
class KillAura : Module() {
    ...
    // Range
    val rangeValue = FloatValue("NormalRange", 3.5f, 3f, 8f)
    val GroundrangeValue = FloatValue("GroundRange", 3.5f, 3f, 8f)
    val AirrangeValue = FloatValue("AirRange", 3.3f, 3f, 8f)
    val KaFix = BoolValue("KaFix",false)
    ...
    private fun updateRotations(entity: IEntity): Boolean {
        ...
        if (rotations.get().equals("hyt", ignoreCase = true)){
            ...
            val (vec, rotation) = RotationUtils.lockView2(
                boundingBox,
    ...
    private fun startBlocking(interactEntity: IEntity, interact: Boolean) {
        ...
        if ((autoBlockValue.equals("Range") && mc.thePlayer!!.getDistanceToEntityBox(interactEntity)> BlockRangeValue.get())||autoBlockPacketValue.get().equals("NewC08", true)) {
            return
        }
        ...
        when(autoBlockPacketValue.get()){
            ...
            "HuaYuTing" ->{
                ...
                if(mc.thePlayer!!.heldItem != null && mc.thePlayer!!.heldItem!!.item is ItemSword){
                    if (mc2.player.swingProgressInt < 0.5 && mc2.player.swingProgressInt != -1) {
